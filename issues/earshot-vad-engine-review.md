...
## 二审(针对 `c36b4f6` + `82dde47`)

作者在初审报告之上追加两个提交,回应了全部 8 项发现。二审实测复验:

- `c36b4f6 docs(fswtch): 代码审查反馈 — 文档补强 + MSRV + 类语音测试夹具` → #3/#5/#7/#8
- `82dde47 feat(fswtch): earshot 引擎 feature 门控(#6)+ #1/#2 固有耦合文档化` → #6/#1/#2

### 二审验证结果(全新 worktree `82dde47` 实跑)

| 配置 | 命令 | 结果 |
|---|---|---|
| 默认(earshot on) | `cargo fmt --all --check` | ✅ 通过 |
| 默认 | `cargo clippy -p fswtch --all-targets` | ✅ `vad.rs` 0 warning |
| 默认 | `cargo test --lib -p fswtch` | ✅ **65 passed; 0 failed** |
| earshot off | `cargo check -p fswtch --no-default-features --features bundled` | ✅ 干净(仅 pkg-config 预期 warning) |
| earshot off | `cargo clippy -p fswtch --all-targets --no-default-features --features bundled` | ✅ `vad.rs` 0 warning,**无** `non_exhaustive`/`dead_code`/`unreachable` |
| earshot off | `cargo test --lib -p fswtch --no-default-features --features bundled` | ✅ **63 passed**(两个 earshot 测试正确缺席) |

### #1 固有耦合 — 实测确认文档准确

`nm -u` 默认 build 的 `libfswtch-*.rlib`,可见 7 个 `switch_vad_*` 外部未定义符号(`_switch_vad_init`/`_switch_vad_process`/`_switch_vad_destroy`/`_switch_vad_get_state`/`_switch_vad_reset`/`_switch_vad_set_mode`/`_switch_vad_state2str`)。即 `Vad` 恒引用 FreeSwitch 符号——即使 earshot-only 调用方运行时不走 FreeSwitch arm,链接层仍需解析这些符号。模块 doc + Cargo.toml feature 注释的表述("drags in"/"references",非"calls")**精确无误**。作者选文档化而非拆函数(方案 A),取舍清晰、可接受。

### 各发现回应核对

| # | 处理 | 二审意见 |
|---|---|---|
| #1/#2 | 文档化为统一枚举的固有耦合,指向"无 FS 需求请直用 earshot crate" | ✅ 文档准确(nm 实证),取舍合理 |
| #3 | `step` doc 补 onset 硬清零注意事项 + 真实语音校准建议 | ✅ 落地 |
| #4 | 220 Hz 纯音 → 类语音夹具(6 谐波 1/k 衰减 ~120 Hz、onset/offset 包络、确定性 LCG ~5% 噪声);仍 `live_fs` 门控并注明"需真实 FS 确认" | ✅ 夹具更接近训练分布;**核心不确定性(夹具能否令 earshot 触发)仍需真实 FS build 验证**,本地无 FS 无法复跑,作者已诚实标注 |
| #5 | `set_param` doc 补 threshold 与 `set_mode` 双量纲关系(`2`→0.002 非 0.2) | ✅ 落地 |
| #6 | `earshot` 改 optional dep + `earshot` feature(默认开),全量 `#[cfg(feature="earshot")]` 门控 variant/常量/`EarshotInner`/`downmix`/各 arm/测试 | ✅ 门控完备:earshot-off 仍穷尽编译、0 warning、63 passed |
| #7 | `process` doc 补 buffer 写行为不对称说明 | ✅ 落地 |
| #8 | 声明 `rust-version = "1.87"`(`crates/fswtch` `[package]`) | ✅ 落地 |

### 值得肯定的细节

- `Drop` 由 `if let` 改 `match`,使 earshot 关闭(单变体)时仍穷尽、无 irrefutable/non-exhaustive 警告——细节到位。
- 类语音夹具用确定性 LCG(`0x5EED` 种子 + glibc 常数),可复现;`synthetic_speech` 门控从 `live_fs` 收紧为 `all(earshot, live_fs)`,避免 earshot-off 时的 dead_code。
- `earshot` 关闭路径下 `VadEngine::default()`(单变体)、`vadengine_default_is_freeswitch`/`snap_*` 测试仍正确运行——两套 feature 配置都验证过。

### 二审结论

**准予合并。** 8 项发现全部落地或以清晰取舍文档化;两套 feature 配置(earshot on/off)均 fmt/clippy/test 通过;#1 耦合经 `nm` 实证、文档表述准确。唯一悬留项是 #4 的真实 FS 回归确认——属环境受限,非代码问题,建议在具备 FreeSWITCH build 的 CI/环境中补跑 `earshot_vad_speech_segments` / `earshot_vad_resampled_8k` 后再发布。
