...
- **CMake `install(TARGETS fswtch_aec3 ARCHIVE DESTINATION .)`.** The `cmake` crate runs
...
`cargo test -p fswtch-aec3 --lib ::cancels_a_real_echo` feeds deterministic broadband-noise render
...
| Phase | Commit | What |
|-------|--------|------|
| 0 | `6f52ce4` | Scaffold `fswtch-aec3-sys` + `fswtch-aec3`; cmake→bindgen→Rust pipeline; smoke. |
