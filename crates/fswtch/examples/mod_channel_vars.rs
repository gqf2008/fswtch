...
//! session backing an API call. The `fswtch_channel_show` command is invoked with a
//! session bound, e.g. from the dialplan as `api fswtch_channel_show`, or from fs_cli
//! against a bridged call.
//!
//! Build a call so the module has a session to inspect:
//!   load mod_channel_vars;
//!   then, with an active channel, run `api fswtch_channel_show`.
//!
//! When invoked without a session (e.g. straight from `fs_cli fswtch_channel_show`
...
//! field, then sets and reads back a made-up variable `fswtch_channel_var_example`.
...
fswtch::api_callback! {
    fn show_api(_cmd, session, stream) {
        fswtch::log_info("mod_channel_vars", "fswtch_channel_show invoked");
        ...
        let Some(session) = session else {
            return stream.write(
                "usage: fswtch_channel_show — invoke from a session (api fswtch_channel_show) so the channel is available\n",
            );
        };
        ...
        match channel.set_variable("fswtch_channel_var_example", "set-by-mod_channel_vars") {
            Ok(()) => {
                let readback = channel
                    .variable("fswtch_channel_var_example")
                    .ok()
                    .flatten()
                    .unwrap_or_else(|| "<readback-unset>".to_owned());
                out.push_str(&format!("fswtch_channel_var_example={readback}\n"));
            }
            Err(error) => {
