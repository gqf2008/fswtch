...
//! ergonomics. The `fswtch_db_lookup` API command opens a private in-memory database, creates a
...
//!   `fswtch_db_lookup`           — runs the round-trip and prints the stored value
//!   `fswtch_db_lookup <text>`     — stores `<text>` instead of the default "fswtch"
...
fswtch::api_callback! {
    fn db_lookup_api(cmd, _session, stream) {
        fswtch::log_info("mod_db_lookup", "fswtch_db_lookup invoked");
