...
- **media bug 路径**：若 voice-call 的 `mod_voice_seat` 用 `MediaBugContext`/`MediaFrame`（fork 的 media bug 生命周期 API），需单独确认回调签名与 fork 对齐 —— 这是 ai-agent-seat（endpoint 模式）没用到的部分，迁移前先验证

## 参考

- fswtch wrapper 源码：`crates/fswtch/src/{session,channel,event,cause}.rs`
- 已迁移的参考实现：`ai-agent-seat/src/control.rs`（CallControl trait + Session locate/hangup/send_dtmf）
- ai-agent-seat 的 `SendResample`/`SendBinder`（`unsafe impl Send` 范例）：`ai-agent-seat/src/audio_dsp.rs`、`event_sub.rs`
